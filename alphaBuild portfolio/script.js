...
const spamDetected = spamWords.some(word =>
...
if (spamDetected) {
  ...
  if (statusEl) {
    ...
    showPopup(
   'error',
   'Spam content detected. Please enter a genuine enquiry.'
);
  }
...
btnSpan.textContent = 'Sending…';
   ...
   const token = await grecaptcha.execute(
      '6LdakXgtAAAAAPZhwoaP2K0FMQyXTAtyOcsetx0g',
      { action: 'contact' }
   );
   ...
   'Thank you! We will contact you within 24 hours.'
        ...
        contactForm.reset(); localStorage.setItem(
   'lastSubmit',
   Date.now()
);
      ...
      if (statusEl) {
        ...
        showPopup(
   'error',
   'Email sending failed. Please contact us on WhatsApp or try again later.'
);      }
